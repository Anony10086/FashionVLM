...
class EvalDataset(Dataset):
    ...
    def __getitem__(self, index):
        image_path = os.path.join(self.root, 'temp', f'{index}_0.jpg')
        image = Image.open(image_path)

        with open(os.path.join(self.root, 'temp', f'{index}.json'), 'r') as f:
...
def collate_fn(batch, uni_prompting=None, img_tokenizer=None):
    ...
    prompt_batch = []
    for x in batch:
        prompt = 'USER: ' + SYSTEM_PROMPT + x['conversations'][0]['value'] + ' ASSISTANT:'
        print(prompt)
...
#############Loading Model#############
#######################################
config = OmegaConf.load("/mnt/d/PostDoc/fifth paper/code/FashionVLM/show_o/configs/showo_fashionrec_tuning_3_512x512.yaml")
...
save_dir = f"/mnt/d/PostDoc/fifth paper/code/FashionVLM/output/{args.method}"
os.makedirs(save_dir, exist_ok=True)
with open(f"{save_dir}/a100_results.json", "w") as f:
